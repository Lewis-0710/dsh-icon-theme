...
> 4. **自动替换通用回退齿轮与真实原图标呈现**：默认策略优化为 `replace-generic`，避免宿主占位齿轮阻塞自动分配；扩充了 Antigravity、会话导入、服务控制、液态玻璃、桌面设置等项的预设；修复图标设置列表中原生专有图标展示（智能体预设、模型、桌面等真实呈现官方原生图标，不再全部画成齿轮）。
>
