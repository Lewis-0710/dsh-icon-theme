...
> 4. **第三方插件专属图标保护与真实原图标呈现**：
>    - **精准识别第三方专有图标**：通过扩展识别专属 DOM 标记（`data-agy-nav-icon`、`data-dsh-chat-import-settings-nav`、`data-dsh-liquid-glass-settings-nav`、`data-dsh-service-nav`、`data-dsh-market-nav-icon` 等），放行第三方插件自绘的高清专属图标（如 Antigravity 官方 A、会话导入托盘、服务控制机架、液态玻璃水滴、插件市场悬浮积木等），严禁篡改与错误覆盖。
>    - **专有原图标优先保护**：在图标解析流水线中将专有原图标（`hasOriginal && !originalIsGeneric`）提升至预设之前，确保各插件官方原汁原味的原生专属设计完好无损。
>    - **真实原图标呈现**：修复图标设置列表中原生/专有图标展示（模型、桌面、桌面通知等真实呈现官方原生图标，不再全部画成齿轮）。
>
